...
import com.mystatus.nachos.databinding.FragmentPlayerBinding;
...
public class CustomPlayerFragment extends Fragment {
    ...
    public static CustomPlayerFragment newInstance(String videoUrl, Boolean isLive, String videoType, String videoTitle, String videoSubTile,
                                                   String videoImage, Integer videoId_, String _videoKind, String seekTime, Boolean isEpisode) {
        ...
        args.putString("seekTime", seekTime);
        args.putBoolean("isEpisode", isEpisode);
    ...
    public void dpad_play() {
        exo_progress.setVisibility(View.VISIBLE);
        if (mCustomPlayerViewModel.isPlaying_custom()) {
            mCustomPlayerViewModel.mExoPlayer.setPlayWhenReady(false);
        } else {
            mCustomPlayerViewModel.mExoPlayer.setPlayWhenReady(true);
        }


    }

    public void show_conroller() {
        mSimpleExoPlayerView.showController();
        exo_progress.setVisibility(View.VISIBLE);
    ...
    public void setSubtitleView() {
        ...
        mSimpleExoPlayerView.getSubtitleView().setStyle(new CaptionStyleCompat(textColor, Color.TRANSPARENT, backrgoundColor, CaptionStyleCompat.EDGE_TYPE_DROP_SHADOW, Color.BLACK, null));
        mSimpleExoPlayerView.getSubtitleView().setFixedTextSize(TypedValue.COMPLEX_UNIT_DIP, 28);
//        relative_layout_dialog_source_background_color_picker.setBackgroundColor(backrgoundColor);
        ...
        text_view_dialog_source_size_text.setText("Text size : " + textSiz + " sp");
    }
...
//        });
    ...
    private void SelectCurrentSubtitle() {
        if (pref.getString("subtitle_enabled").equals("FALSE")) {
            if (subtitles.size() > 0) {
                Subtitle currentSubtitle = subtitles.get(0);
                subtitles.get(0).setSelected(true);
                for (int i = 0; i < subtitles.size(); i++) {
...
//        done =  true;
    ...
    public class LanguageAdapter extends RecyclerView.Adapter<LanguageAdapter.LanguageHolder> {
        ...
        public void onBindViewHolder(LanguageHolder holder, final int position) {
            ...
            ImageLoader.with(getActivity()).load(languages.get(position).getImage()).into(holder.image_view_item_language);
...
//            });
    ...
    public void onDestroy() {
        ...
        long current = mCustomPlayerViewModel.mExoPlayer.getCurrentPosition();
        long duration = mCustomPlayerViewModel.mExoPlayer.getDuration();
        int percent = (int) ((double) ((float) current / (float) duration) * 100);
        updateWatchTime(String.valueOf(mCustomPlayerViewModel.mExoPlayer.getCurrentPosition()), percent);
    }

    public long getSeekTime() {
        return mCustomPlayerViewModel.mExoPlayer.getCurrentPosition();
    }
    ...
    public void updateWatchTime(String milis, Integer percentage) {
        ...
        if (getActivity().getIntent().getBooleanExtra("isEpisode", false)) {
            episode_id = getActivity().getIntent().getIntExtra("id", 0);
            call = service.updateWatchTime(email, null, episode_id, milis, percentage, final_last_update, 0);
        } else {
            poster_id = getActivity().getIntent().getIntExtra("id", 0);
            call = service.updateWatchTime(email, poster_id, null, milis, percentage, final_last_update, 0);
        }
