...
import com.mystatus.nachos.ui.Adapters.IntroAdapter;
import com.mystatus.nachos.ui.views.ClickableViewPager;
...
public class IntroActivity extends AppCompatActivity {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == android.view.KeyEvent.KEYCODE_DPAD_CENTER) {
            if (text_view_next_done.getText().equals("לחץ אישור בשלט להמשך")){
                Intent intent;
                if(prefManager.getString("LOGGED").toString().equals("TRUE")){
                    intent = new Intent(IntroActivity.this, HomeActivity.class);
                }else{
                    intent = new Intent(IntroActivity.this, LoginNewActivity.class);
                }
        ...
        view_pager_slide.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                if (position+1==slideList.size()){
                    text_view_next_done.setText("לחץ אישור בשלט להמשך");
                }else{
                    text_view_next_done.setText("הבא");
                }
            }
