...
public class DpadAwareRecyclerView extends RecyclerView implements
    ...
    /* Selector attributes */
    private boolean interceptTopBottomKeyEvent;
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
        if (attrs != null) {
            ...
            interceptTopBottomKeyEvent = ta.getBoolean(R.styleable.DpadAwareRecyclerView_interceptTopBottomScroll, false);
    ...
    public boolean dispatchKeyEvent(@NonNull KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT
            ...
            if (!canMove || isLongPress) return true;
            canMove = false;
            new Handler().postDelayed(() -> canMove = true, 100);
        }

        if (interceptTopBottomKeyEvent) {
